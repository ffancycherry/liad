...
export const loadAllEventsFx = createEffect(
  async ({ limit, offset, additionalParam }: ILoadAllEventsFx) => {
    try {
      const { data } = await api.get(
        `/api/events?limit=${limit}&offset=${offset}&${additionalParam}`
      )
...
export const $events = events
  ...
  .on(loadAllEventsFx.done, (_, { result }) => result)

sample({
  clock: loadAllEvents,
  source: $events,
  fn: (_, data) => data,
  target: loadAllEventsFx,
})
